            ...
            <div class="relative flex justify-center text-white p-6 px-0">
                <div class="w-11/12 md:w-full">
                    ...
                    <div class="w-full lg:absolute lg:right-0 lg:top-0 lg:w-1/2 md:h-full lg:h-full overflow-hidden rounded-md shadow-md">
                        <img src="{{ Vite::asset('resources/images/dystopie.png') }}"
                ...
                <form class="mt-8" action="{{ route('daily-question.submit') }}" method="post">
                    ...
                    <fieldset>
                        ...
                        <div class="text-text-dark text-lg">
                            ...
                            <div class="mb-3">
                                <input type="radio" id="option{{ $key }}" name="answer" value="{{ $key }}" class="mr-2">
                                <label for="option{{ $key }}">{{ $key }}. {{ $option }}</label>
                            </div>
                            @endforeach
                        </div>
                        <button type="submit" class="hidden group flex justify-center items-center gap-2 mt-4 px-4 py-2 btn-primary rounded transition-colors duration-500 ease-in-out">Antwoord indienen <svg class="w-4 h-4 inline fill-current group-hover:translate-x-1 transition-transform ease-in-out duration-500" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M15.92 7.12a1 1 0 0 0-.22-.33l-4.94-4.95a1 1 0 0 0-1.42 1.42l3.25 3.24H1a1 1 0 0 0 0 2h11.59l-3.25 3.24a1 1 0 1 0 1.41 1.41L15.7 8.2a1 1 0 0 .22-1.09z"/></svg></button>
                    </fieldset>
                </form>
            </section>
        </div>
